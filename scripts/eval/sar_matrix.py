"""Non-optical modality control. Identical matched-input matrix to
...
SARDIR = REPO / "data/raw/sentinel1"
...
def load_region(region):
    ...
    for npz_path in glob.glob(str(SARDIR / "*.npz")):
        ...
        patches = d["patches"]                         # (n,2,64,64) float32 VV,VH
        ...
        for k, tid in enumerate(tile_ids):
            ...
            if tgt.shape != (GRID, GRID):
                continue
            sar = patches[k].astype(np.float32)        # (2,64,64)
            if sar.shape[-1] != GRID:
                sar = zoom(sar, (1, GRID / sar.shape[-2], GRID / sar.shape[-1]), order=1)
            sar = sar[:, :GRID, :GRID]
            if sar.shape != (2, GRID, GRID):
                continue
...
def main(seeds, grid):
    ...
    out = {"input": "Sentinel-1 RTC SAR (VV+VH)", "target": "GHSL built-up 2015",
           "grid": grid, "seeds": list(seeds), "per_seed": {str(sd): per[sd] for sd in seeds},
           "aggregate": agg}
    fn = REPO / f"results/metrics/sar_matrix_g{grid}_multiseed.json"
    json.dump(out, open(fn, "w"), indent=1)
    print(f"\n=== Sentinel-1 SAR grid={grid}, {len(seeds)} seeds ===")
    ...
    print("  compare: GHSL-history ~1.0 ; raw Landsat 0.56 ; single-date S2 0.82", flush=True)
