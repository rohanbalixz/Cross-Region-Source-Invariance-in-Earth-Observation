"""A second harmonized product (ESA WorldCover, a different
upstream classifier than JRC/GHSL) -> GHSL built-up 2015. If retention ~1.0, a
harmonized product from two independent sources transfers, the positive arm is
not specific to one product. Input: 6 one-hot land-cover channels. Matched-input matrix to
...
SARDIR = REPO / "data/raw/worldcover_aligned"   # WorldCover one-hot (n,6,64,64)
EXCLUDE_BUILTUP = False                          # --exclude-builtup: non-circular test (drop built-up input channel)
...
def load_region(region):
    ...
    for npz_path in glob.glob(str(SARDIR / "*.npz")):
        ...
        patches = d["patches"]                         # (n,6,64,64) WorldCover one-hot
        ...
        for k, tid in enumerate(tile_ids):
            ...
            if tgt.shape != (GRID, GRID):
                continue
            sar = patches[k].astype(np.float32)        # (6,64,64) [tree,crop,BUILT,water,grass,bare]
            if EXCLUDE_BUILTUP:                         # non-circular: drop the built-up channel (idx 2)
                sar = np.delete(sar, 2, axis=0)
            if sar.shape[-1] != GRID:
...
def main(seeds, grid):
    ...
    out = {"input": "ESA WorldCover land-cover (2nd harmonized product)", "target": "GHSL built-up 2015",
           "grid": grid, "seeds": list(seeds), "per_seed": {str(sd): per[sd] for sd in seeds},
           "aggregate": agg}
    _sfx = "_nobuilt" if EXCLUDE_BUILTUP else ""
    fn = REPO / f"results/metrics/worldcover_builtup_g{grid}{_sfx}_multiseed.json"
    json.dump(out, open(fn, "w"), indent=1)
    print(f"\n=== WorldCover->built-up grid={grid}, {len(seeds)} seeds ===")
    ...
    print("  compare: GHSL-history ~1.0 ; NDVI(uniform meas) 0.77 ; raw Landsat 0.56", flush=True)
...
if __name__ == "__main__":
    ...
    p.add_argument("--exclude-builtup", action="store_true")
    a = p.parse_args()
    GRID = a.grid
    EXCLUDE_BUILTUP = a.exclude_builtup
    main(a.seeds, a.grid)
